    ...
    void fileWriter(int per) {
        ...
        try {
                ...
                for (Node j : nodes) {
                    if (j.name.equals(String.valueOf(whichNode))) {
                        ...
                        for (Properties p : j.properties) {
                            if (counter % per == 0) {
                                String string = String.format("t = %.6f , V = %.6f , ", p.time, p.voltage);
                                out.write(string);
                            }
                            counter++;
                        }
                        out.write("\n");
                        String string = String.format("final results: t = %.6f , V = %.6f , ", j.properties.get(j.properties.size() - 1).time, j.properties.get(j.properties.size() - 1).voltage);
                        out.write(string);
                    }
                }
                whichNode++;
            ...
            for (TwoPort e : elements) {
                ...
                out.write("\n");
                String string = String.format("final results: t = %.6f , V = %.6f , I = %.6f , P = %.6f , ", e.properties.get(e.properties.size() - 1).time, e.properties.get(e.properties.size() - 1).voltage, e.properties.get(e.properties.size() - 1).current, e.properties.get(e.properties.size() - 1).power);
                out.write(string);
                out.write("\n");
            }
